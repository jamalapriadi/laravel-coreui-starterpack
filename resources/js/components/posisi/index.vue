    ...
    <div class="card card-accent-primary">
        <div class="card-header">
            ...
            <div class="card-header-actions">
                <router-link to="/posisi/create" class="btn btn-primary">
                ...
                <table class="table table-striped">
                    ...
                    <tbody>
                        <tr v-for="(l, index) in list.data" v-bind:key="index">
                            <td>{{index+1}}</td>
                            <td>
                                <label v-for="(k,idx) in l.jabatan" :key="idx">{{k.name}}</label>
                            </td>
                            <td>{{l.name}}</td>
                            <td>
                                <div class="btn-group">
                                    <router-link :to="{ name: 'posisi-view', params: {id: l.id}}" class="btn btn-warning">
    ...
    export default {
        ...
        methods: {
            ...
            showData(page){
                ...
                axios.get('data/posisi?page='+page)
                    .then(response => {
                        this.loading=false;
                        this.list = response.data;
